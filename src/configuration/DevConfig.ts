/* eslint-disable import/no-anonymous-default-export */

export default {
    DOMAIN: process.env.NEXT_PUBLIC_DEV_DOMAIN,
    API_BASE_URL: `${process.env.NEXT_PUBLIC_DEV_API_BASE}/graphql`,
    // API_BASE_URL: "https://api.piratemobile.gg/graphql",
    GOOGLE_CLIENT_ID:
        "1060585065695-kl1dpg1pv18kq6vvuqsjos19laptjc9g.apps.googleusercontent.com",
    GOOGLE_CLIENT_SECRET: "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",
    PAYPAL_CLIENT_ID:
        "AXcQh-HAlPuWKMa-1jIB5b-IVs5Qs_dl5MmBHKQEJTQYKh-K9w-tipJ2I9YOXTGG_SVQSCfqdfY2Zpjs",
    PAYPAL_SECRET_CODE:
        "EIRtY98U_vi2XBwZJxGU9n4-f0cu-xOqVaTjsqwouFCJAsh9lgQoySt1BNMRX-hdGUOE3h0ftHoxz_ex",
};
